...
local Math = require(MainStorage.Common.Math)
local Vec2 = Math.Vec2
local Vec3 = Math.Vec3
local Vec4 = Math.Vec4
local Quat = Math.Quat
local Mat4 = Math.Mat4x4
local Mat3x4 = Math.Mat3x4
local MathDefines = Math.MathDefines

local ShakeController = require(MainStorage.Camera.ShakeController)
...
function CameraController.New()
    local ret = {}
    for k, v in pairs(CameraController) do
        if k ~= 'New' then
            ret[k] = v
        end
    end
...
function CameraController:Init()
    ...
    self.shakeCtrl = ShakeController
    --摄像机模式
    self.cameraMode = self.CameraMode.ThirdPerson
    --摄像机跟随的目标偏移
    self.offset = Vec3.new(-140, 140, -140)
    ...
    --灵敏度
    self.mouseXSensitivity = 0.15
    self.mouseYSensitivity = 0.15
    ...
    self.mouseYMax = 90.0
    --平滑时间
    self.mouseSmoothTime = 0
    --距离限制
    self.minDistance = 50
    self.maxDistance = 2000
    --距离平滑时间
    self.distanceSmoothTime = 0.5
    --移动对齐
    ...
    self._mouseX = 10
    -- 这个地方可以改变植物的初始视角
    self._mouseY = self.owner.bindContainer.Euler.Y + 180

    -- 后坐力的偏移
    self._mouseXOffset = 0
    self._mouseYOffset = 0
...
function CameraController:StartClient()
    ...
    elseif UserInputService.MouseEnabled then -- 鼠标
        self.InputBeganEvent =
            ...
            function(inputObj, gameprocessed)
                if
                    inputObj.UserInputType == Enum.UserInputType.MouseButton1.Value or inputObj.UserInputType == Enum.UserInputType.MouseButton2.Value or
...
function CameraController:SetMouseSmoothTime(smoothTime)
    if not ShakeController:IsShaking() then
...
function CameraController:ThirdPersonUpdate(dt)
    ...
    self._mouseXSmooth, self._mouseXCurrentVelocity =
        Math:SmoothDamp(self._mouseXSmooth, self._mouseX + self._mouseXOffset, self._mouseXCurrentVelocity, self._mouseSmoothTime, 0, dt)
    self._mouseYSmooth, self._mouseYCurrentVelocity =
        Math:SmoothDamp(self._mouseYSmooth, self._mouseY + self._mouseYOffset, self._mouseYCurrentVelocity, self._mouseSmoothTime, 0, dt)
    -- 计算摄像机旋转
    ...
    if not isHit then
        -- 如果没有碰撞，平滑处理摄像机距离
        self._distanceSmooth, self._distanceCurrentVelocity = Math:SmoothDamp(self._distanceSmooth, closestDistance, self._distanceCurrentVelocity, self.distanceSmoothTime, 0, dt)
    else
        -- -- 如果发生碰撞，直接设置距离并重置速度
    ...
    self.camera.Rotation = Quaternion.New(self.currentCameraRot.x, self.currentCameraRot.y, self.currentCameraRot.z, self.currentCameraRot.w)
...
function CameraController:GetClosestDistance(pos, orient)
    ...
    local result = WorldService:SweepSphere(50, Vector3.New(pos.x, pos.y, pos.z), Vector3.New(dir.x, dir.y, dir.z), self._distance, true, {2})
...
function CameraController:InputMove(deltaX, deltaY)
    ...
    --限制角度
    self._mouseX = math.clamp(self._mouseX + self._mouseXOffset, self.mouseXMin, self.mouseXMax)
    self._mouseX = self._mouseX - self._mouseXOffset -- 去除 Offset 影响
    -- self._mouseY = math.clamp(self._mouseY, self.mouseYMin, self.mouseYMax)
end

function CameraController:ApplyRecoil(pitch, yaw)
    self._mouseXOffset = pitch
    self._mouseYOffset = yaw
end

function CameraController:ResetRecoil()
    if self._mouseX == nil or self._mouseY == nil then
        return
    end

    self._mouseX = self._mouseX + self._mouseXOffset
    self._mouseY = self._mouseY + self._mouseYOffset
    self._mouseXOffset = 0
    self._mouseYOffset = 0
end
